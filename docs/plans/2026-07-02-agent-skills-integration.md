...
**Architecture:** Thin-wrapper approach — SKILL.md files copied verbatim from `agent-skills`, renamed with `h-` prefix; no logic changes to existing pipeline skills or `sdd-lib.sh`; three one-line companion notes added to `h-sdd-plan`, `h-sdd-analyze`, and `h-sdd-implement`.
...
### Files created
- `h-security-and-hardening/SKILL.md` — verbatim copy from `agent-skills`
- `h-performance-optimization/SKILL.md` — verbatim copy from `agent-skills`
- `h-debugging-and-error-recovery/SKILL.md` — verbatim copy from `agent-skills`
- `h-api-and-interface-design/SKILL.md` — verbatim copy from `agent-skills`
- `h-frontend-ui-engineering/SKILL.md` — verbatim copy from `agent-skills`

### Files modified
- `vendor.sh` — add `== 1b. copy from agent-skills ==` block (lines after existing `== 1. copy ==`)
- `install.sh` — add `--project <path>` flag handling
- `h-sdd-plan/SKILL.md` — add optional-companion note at end of Phase 2
- `h-sdd-analyze/SKILL.md` — add optional-companion note at end of Phase 1
- `h-sdd-implement/SKILL.md` — add optional-companion note at end of Phase 0
...
**Files:**
- Create: `h-security-and-hardening/SKILL.md`
- Create: `h-performance-optimization/SKILL.md`
- Create: `h-debugging-and-error-recovery/SKILL.md`
- Create: `h-api-and-interface-design/SKILL.md`
- Create: `h-frontend-ui-engineering/SKILL.md`
...
Expected output:
```
copied h-security-and-hardening
copied h-performance-optimization
copied h-debugging-and-error-recovery
copied h-api-and-interface-design
copied h-frontend-ui-engineering
```
...
```bash
git add h-security-and-hardening/ h-performance-optimization/ \
        h-debugging-and-error-recovery/ h-api-and-interface-design/ \
...
```
== 1. copy ==
  vendored brainstorming -> h-brainstorming
  ...
== 1b. copy from agent-skills ==
  vendored security-and-hardening -> h-security-and-hardening
  vendored performance-optimization -> h-performance-optimization
  vendored debugging-and-error-recovery -> h-debugging-and-error-recovery
  vendored api-and-interface-design -> h-api-and-interface-design
  vendored frontend-ui-engineering -> h-frontend-ui-engineering
...
mkdir -p "$TMPDIR/.devin/skills/h-security-and-hardening"
bash install.sh --project "$TMPDIR" 2>&1 | grep "security"
```

Expected: `skip h-security-and-hardening (already exists in project — left untouched)`
...
**Files:**
- Modify: `h-sdd-plan/SKILL.md` — append to Phase 2 section
- Modify: `h-sdd-analyze/SKILL.md` — append to Phase 1 section
- Modify: `h-sdd-implement/SKILL.md` — append to Phase 0 section

- [ ] **Step 1: Add companion note to `h-sdd-plan/SKILL.md`**

At the end of the `## Phase 2 — Companions, approval gate & route` section (after the `- Route to \`h-sdd-tasks\`.` line), add:
...
- [ ] **Step 2: Add companion note to `h-sdd-analyze/SKILL.md`**
...
- [ ] **Step 3: Add companion note to `h-sdd-implement/SKILL.md`**
...
```bash
wc -l h-sdd-plan/SKILL.md h-sdd-analyze/SKILL.md h-sdd-implement/SKILL.md
```

Expected: each file's line count is its original count + 2 (the new blockquote line + a blank line before it).

Original counts: `h-sdd-plan` = 30 lines → 33; `h-sdd-analyze` = 36 lines → 39; `h-sdd-implement` = 28 lines → 31.

- [ ] **Step 5: Commit**

```bash
git add h-sdd-plan/SKILL.md h-sdd-analyze/SKILL.md h-sdd-implement/SKILL.md
git commit -m "feat(sdd): add optional quality companion hints to plan/analyze/implement

h-sdd-plan references h-api-and-interface-design and h-frontend-ui-engineering.
h-sdd-analyze references h-security-and-hardening and h-performance-optimization.
h-sdd-implement references h-debugging-and-error-recovery.
...
- [ ] `h-sdd-plan/SKILL.md` mentions `h-api-and-interface-design` and `h-frontend-ui-engineering`
- [ ] `h-sdd-analyze/SKILL.md` mentions `h-security-and-hardening` and `h-performance-optimization`
- [ ] `h-sdd-implement/SKILL.md` mentions `h-debugging-and-error-recovery`
- [ ] `h-sdd/test_sdd_lib.sh` passes
- [ ] `sdd-lib.sh` is unchanged
- [ ] `h-sdd/_shared.md` is unchanged
