...
- `m-sdd/_shared.md:1-11` — delegate rule; new skills are named companions, not
  delegates
- `m-sdd/_shared.md:38-55` — HITL gate rules; new skills must never be added as
  `sdd_require` gates
...
|---|---|---|
| `m-sdd-plan/SKILL.md` | End of phase, after plan artifact drafted | `m-api-and-interface-design` (if spec involves an API), `m-frontend-ui-engineering` (if spec involves UI) |
| `m-sdd-analyze/SKILL.md` | In cross-check section, before analyze verdict | `m-security-and-hardening`, `m-performance-optimization` |
| `m-sdd-implement/SKILL.md` | In failure/recovery section | `m-debugging-and-error-recovery` |
...
(`m-sdd`, `m-sdd-specify`, etc.) are global-only.
...
- [ ] `m-sdd-plan/SKILL.md` mentions `m-api-and-interface-design` and
      `m-frontend-ui-engineering` as optional companions
- [ ] `m-sdd-analyze/SKILL.md` mentions `m-security-and-hardening` and
      `m-performance-optimization` as optional companions
- [ ] `m-sdd-implement/SKILL.md` mentions `m-debugging-and-error-recovery`
      as an optional companion
- [ ] No existing tests in `m-sdd/test_sdd_lib.sh` break
...
**Approach B — Deep integration:** Rewrite `m-sdd-analyze` to actively invoke
