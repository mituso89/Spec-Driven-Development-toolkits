...
> **TL;DR** — Draft `.sdd/knowledge.md` from repo discovery + context7 **and generate the `.sdd/repo-map.md` structural layer** (1) → user curates & write, recording the structural-grounding source (2) → route to specify (3). Read `~/.config/devin/skills/h-sdd/_shared.md` first.
...
- `source ~/.config/devin/skills/h-sdd/sdd-lib.sh; root="$(pwd)"`; ensure scaffold: `sdd_scaffold "$root"` (copies `knowledge-template.md` into `.sdd/templates/`).
- No hard gate — knowledge is project-level and optional. If `.sdd/constitution.md` is absent, suggest `h-sdd-constitution` but proceed.
...
- **Structural map (generated):** run the repo-map generator to capture files +
  ...
  `bash ~/.config/devin/skills/h-sdd-knowledge/repo-map.sh "$root"` → writes
...
## Phase 3 — Route
- Point to `h-sdd-specify` (next step), or `h-sdd-constitution` if rules aren't set yet.
