---
name: h-sdd-plan
description: "SDD phase 4 — produce the technical implementation plan by delegating to the h-writing-plans skill, writing specs/<id>/plan.md (+ research/data-model/contracts). Triggers: sdd plan, write sdd plan, plan feature sdd, implementation plan sdd, technical plan."
---

# SDD — Plan

> **TL;DR** — Gate on an approved spec (0) → delegate to **h-writing-plans**, redirecting output to `specs/<id>/plan.md` (1) → add companion artifacts, **get human approval & mark `approved`**, route (2). Read `~/.config/devin/skills/h-sdd/_shared.md` first.
...
## Phase 1 — Delegate to h-writing-plans
- `sdd_set_phase "$root" "$id" plan in_progress`
- Invoke **h-writing-plans** with two overrides:
  ...
  2. **Do NOT offer the execution handoff** at the end — implementation is the separate `h-sdd-implement` phase. Return control here.
