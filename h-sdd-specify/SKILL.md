...
> solutions, architectures, or approaches (that's `h-sdd-plan`). If the user still
> needs to *explore* the problem or weigh options before committing to requirements,
> that's an upstream activity: point them at **h-ask** (design-first / brainstorm
> mode) first, then return here to specify. Do **not** delegate this phase to
> `h-brainstorming` — its solution-oriented spine (propose approaches → design
...
- **Banned from spec.md** (they belong in `plan.md`, phase `h-sdd-plan`): tech
...
## Phase 0 — Preflight
- Read `_shared.md`; load constitution (`.sdd/constitution.md`). If absent, warn and offer `h-sdd-constitution`.
- Load `.sdd/knowledge.md` if present (project facts/glossary/pointers) to ground the spec; if absent, note it and suggest `h-sdd-knowledge` (non-blocking).
- `source ~/.config/devin/skills/h-sdd/sdd-lib.sh; root="$(pwd)"; sdd_scaffold "$root"` (idempotent — makes cold-starting here on a fresh repo safe), then `id="$(sdd_active_feature "$root")"`.
...
- Open `.sdd/templates/spec-template.md` and fill **every** section into `specs/$id/spec.md` — Problem & Goal, Non-Goals, User Stories (prioritized, Given/When/Then), Functional Requirements, Key Entities, Success Criteria, Assumptions, Edge Cases. Leave `## Clarifications` empty (it's `h-sdd-clarify`'s).
...
- **Terminal state: the spec file.** Do NOT propose a solution, write a plan, or invoke any implementation/planning skill. Planning is a separate phase (`h-sdd-plan`). Return control here once the spec is written.
...
- Generate `specs/$id/checklists/requirements.md` following **h-sdd-checklist
...
- If the spec carries `[NEEDS CLARIFICATION]` markers, present them **one at a
  time as sequential questions** (use `h-ask` patterns); replace each marker
...
- Route: suggest `h-sdd-clarify` (optional, for deeper ambiguity sweeps) or
  `h-sdd-plan`; mention `h-sdd-checklist` for extra domains (ux, security, …).
