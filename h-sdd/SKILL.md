...
> **TL;DR** — Scaffold `.sdd/` (1) → create or select a feature (2) → show status & route to the next `h-sdd-<phase>` skill (3). Read `_shared.md` for the rules every phase shares.

This is the entry point. Phase skills (`h-sdd-constitution` … `h-sdd-tasks-to-issues`) do the per-phase work; this skill sets up state and tells you where you are.

## Phase 0 — Preflight
- Read sibling `h-sdd/_shared.md`.
...
- `source ~/.config/devin/skills/h-sdd/sdd-lib.sh; root="$(pwd)"`
...
- Tell the user which skill to run next: `h-sdd-<next>` (from `sdd_get_next`).
- If `Next: constitution` and no `.sdd/constitution.md`, recommend `h-sdd-constitution`.
- After the constitution, mention the optional `h-sdd-knowledge` (builds `.sdd/knowledge.md` — project facts that ground specify/clarify); it is not a tracked phase, so it won't appear in `sdd_status`.
- After specify, mention the optional `h-sdd-checklist` (requirement-quality checklists in `specs/<id>/checklists/` — "unit tests for English"); also untracked.
- If the user has UI designs made elsewhere (e.g. in an AI chat tool), note they can drop the HTML into `specs/<id>/design/` — `h-sdd-specify`/`h-sdd-plan` will index and reference it (visual reference, not a tracked phase).
