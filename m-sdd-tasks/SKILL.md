...
> **TL;DR** — Gate on an **approved** plan (0) → extract the plan's checkbox tasks into `specs/<id>/tasks.md` with stable IDs (1) → mark done & route (2). Read `~/.config/devin/skills/m-sdd/_shared.md` first.
...
- Read `specs/$id/plan.md`. `m-writing-plans` already emits bite-sized `- [ ]` steps; collect them into an ordered checklist in `specs/$id/tasks.md` (start from `.sdd/templates/tasks-template.md`).
